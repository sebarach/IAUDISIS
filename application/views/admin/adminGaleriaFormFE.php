		...
		<main class="main" style="height: 100%">
			...
			<div class="container-fluid">
				<div class="animated fadeIn">
					<h4 class="text-theme">Galería Fotográfica Formularios - <?php echo $tr; ?></h4>
					<div class="row" >
			        ...
			        <div class="row" >
			        	<form class="col-md-11" id="formfoto" method="POST" action="<?php echo site_url(); ?>Adm_ModuloFormulario/galeriaform/<?php echo $link;?>?view=<?php echo $view; ?>" >
			        		...
			        		<div class="row">
				        		...
				        		<div class="col-md-2 column-3">
				        			<select class="form-control select" id="usuario" name="usuario" style="width: 100%">
				        				<?php
					        				if($usuario==0){ echo '<option value="" >Usuario</option>'; }
				        				?>
				        			</select>
				        		</div>
			        	...
			        	<div class="col-md-1" style="padding-left: 0px;">
			        		<a title="Borrar Búsqueda" class="btn btn-sm btn-theme pull-left" href="<?php echo site_url(); ?>Adm_ModuloFormulario/galeriaform/<?php echo $link;?>?view=<?php echo $view; ?>" ><i class="fa fa-refresh"></i></a>
			        	</div>
			        </div>
			        <div class="row " >
		     			<div class="col-md-12">
...
<script type="text/javascript">
	...
	$(function(){
	    ...
	    <?php
	    	if(isset($canal)){
				$filtros=array("form","zona","territorio","depto","provincia","distrito","cadena","canal","local","usuario");
				echo 'var fil="&form='.$form.'&zona='.$zona.'&territorio='.$territorio.'&depto='.$depto.'&provincia='.$provincia.'&distrito='.$distrito.'&cadena='.$cadena.'&local='.$local.'&fecha_registro='.$fecha.'&usuario='.$usuario.'&canal='.$canal.'";';
			} else {
				$filtros=array("form","zona","territorio","depto","provincia","distrito","cadena","local","usuario");
				echo 'var fil="&form='.$form.'&zona='.$zona.'&territorio='.$territorio.'&depto='.$depto.'&provincia='.$provincia.'&distrito='.$distrito.'&cadena='.$cadena.'&local='.$local.'&fecha_registro='.$fecha.'&usuario='.$usuario.'";';
			}
			foreach ($filtros as $f) {
				echo '$.post("'.site_url().'Adm_ModuloFormulario/listar'.$f.'","tr='.$link.'&v='.$view.'"+fil,function (data){';
	    ...
	    $("#rad_reporte1").click(function(){
	    	if($('#rad_reporte1').is(':checked')){
				$("#formreport").attr('action','<?php echo site_url(); ?>Adm_ModuloFormulario/pdf/<?php echo $link;?>');
			} 	    	
	    });

	    $("#rad_reporte2").click(function(){
	    	if($('#rad_reporte2').is(':checked')){
				$("#formreport").attr('action','<?php echo site_url(); ?>Adm_ModuloFormulario/powerpointform/<?php echo $link;?>');
			} 	    	
	    });

	    $('#lb_formulario').select2({
	     	ajax:{
				url:"<?php echo base_url("Adm_ModuloFormulario/formularios"); ?>",
				type: "POST",
				data:"tr=<?php echo $link; ?>&v=<?php echo $view; ?>",
				dataType:"json",
				delay: 350,

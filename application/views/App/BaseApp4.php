...
<?php if (!isset($JornadaUser)) {
      echo "  <div class='property-details'>
                <div class='clearfix'>
                  <div class='card-body p-3 clearfix'>
                    ...
                    <div class='h5 text-danger mb-0 mt-2'>No tiene Horarios Asignados!</div>
                      <div class='text-muted text-uppercase font-weight-bold font-xs'>
                        <small>Favor de contactarse con su Supervisor lo antes posible</small>
                      </div>
                    </div>                                                                        
                  </div>
                </div>";
  ...
  $Edate2=date_format($Edate,"H:i:s A");
  $Sdate=date_create($JornadaUser["salida"]);
  $Sdate2=date_format($Sdate,"H:i:s A");
  // $Hentrada=date('h:i:s A', $JornadaUser["entrada"]); 
        echo"     <div class='rent-details'>
                    <div class='clearfix'>
                      <div class='float-left text-dark'>
                        ...
                        <button style='Display:none;' type='button' class='btn btn-outline-theme'><i class='mdi mdi-map-marker-circle'></i></button>
                          <button id='npdv' style='Display:none;' type='button' class='btn btn-outline-theme sweet-7'>¿No es tu Punto?<i class='mdi mdi-map-marker-circle'></i></button>
                        ...
                        <button id='btn-MarcarColacion' style='display: none;' class='btn btn-theme  sweet-5'><i id='incMCola' class='mdi mdi-food-fork-drink'></i> Colación</button>
                        
                      </div>
                    </div>
                  </div>
                </div>
                <div class='btn-group col-md-offset-5' role='group' aria-label='Basic example' style='padding-left:5%; padding-right:5%;'>
                                
                                  <button  id='back' class='btn btn-outline-theme btn-sm'>Ver Locales</button>&nbsp;
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyB3412Cmx6-Q9TR0Zqad70sXtCUhb3505A&callback=initMap"></script>
<script type="text/javascript">
   ...
   $( document ).ready(
          ...
          function localizacion(posicion){
           ...
           if (m<rango) {
              ...
              if (infoEntrada!='' && infoSalida!='' && infoEntradaC!='' && infoSalidaC!='' ) {
                ...
                $("#lbmarca").html('Su asistencia para hoy ya esta completa para este local. Aprete el boton "Ver Locales" para ver sus oto PDV asignados ');
                $("#btn-MarcarAsistencia").attr("style","display:none;");
              }
    ...
    $('#back').click(function(){ 
      var FechaSalida='<?php echo $JornadaUser["FechaSalida"]; ?>';
      var FechaEntrada='<?php echo $JornadaUser["FechaEntrada"]; ?>';
      if (FechaSalida=='' && FechaEntrada!='') {
        swal({
              title: "Error",
              text: "Tiene que Cerrar su Asistencia en este local para volver",
              type: "error",
              showCancelButton: false,
              confirmButtonClass: 'btn-danger',
              confirmButtonText: 'Aceptar'
        ...
        window.location.href='<?php echo base_url("Adm_ModuloAsistencia/Asistencia");?>';  
      }
    });
        ...
        function () {
          ...
          if ($("#latitud1A").val()!='' && $("#longitud1A").val()!='' && $("#idHor").val()!='' && $("#idJor").val()!='') {
            $.ajax({                        
             ...
             {
               if (data==1) {
                swal("Colación Registrada", "Se a iniciado su colación con exito.", "success");                
                setTimeout(function(){window.location.reload();},1000);               
              } else if (data==2){
                swal("Colación Registrada", "Se a Finalizado su colación con exito.", "success");
                setTimeout(function(){window.location.reload();},1000);               
              }else if (data==3){
